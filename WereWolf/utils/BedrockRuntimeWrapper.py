...
class BedrockRuntimeWrapper:
    ...
    def invoke_claude3(self, prompt):
        """
        ...
        try:
            ...
            logger.error("Couldn't invoke Anthropic Claude3")
            raise  
            
    async def invoke_claude3_with_response_stream(self, prompt):
        """
        Invokes the Anthropic Claude 3 model to run an inference using the input
        provided in the request body.
    ...
    # snippet-start:[python.example_code.bedrock-runtime.InvokeAnthropicClaude3]
    
    # snippet-end:[python.example_code.bedrock-runtime.InvokeAnthropicClaude3]
...
def invoke(wrapper, model_id, prompt, style_preset=None):
    ...
    try:
        if model_id == "anthropic.claude-v2":
            ...
            completion = wrapper.invoke_claude3(prompt)
